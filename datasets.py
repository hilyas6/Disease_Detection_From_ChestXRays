...
class XRaysTrainDataset(Dataset):
    ...
    def get_train_val_df(self):
        ...
        train_val_df = pd.concat(rows, axis=1).T if rows else pd.DataFrame()
    ...
    def get_df(self):
        csv_path = os.path.join(self.data_dir, 'Data_Entry_2017.csv')  # Path to the metadata file
        print('\n{} found: {}'.format(csv_path, os.path.exists(csv_path)))

        if not os.path.exists(csv_path):
            raise FileNotFoundError(f"CSV file not found at {csv_path}. Ensure the data folder structure is correct.")

        all_xray_df = pd.read_csv(csv_path)

        df = pd.DataFrame()
        df['image_links'] = [x for x in glob.glob(os.path.join(self.data_dir, 'images*', '*', '*.png'))]
        df['Image Index'] = df['image_links'].apply(lambda x: os.path.basename(x))
    ...
    def get_train_val_list(self):
        f = open(os.path.join(self.data_dir, 'train_val_list.txt'), 'r')  # Use self.data_dir
...
class XRaysTestDataset(Dataset):
    ...
    def get_test_df(self):
        ...
        # Use pd.concat to build the DataFrame
        test_df = pd.concat(rows, axis=1).T if rows else pd.DataFrame()
        print('test_df.shape: ', test_df.shape)
        return test_df

    def get_test_list(self):
        f = open(os.path.join(self.data_dir, 'test_list.txt'), 'r')  # Use self.data_dir
