...
def fit(device, XRayTrain_dataset, train_loader, val_loader, test_loader, model,
        ...
        lr, bs, stage, test_only=False, scheduler=None):
    '''
    ...
    '''
    epoch_train_loss, epoch_val_loss, total_train_loss_list, total_val_loss_list = losses_dict['epoch_train_loss'], \
    losses_dict['epoch_val_loss'], losses_dict['total_train_loss_list'], losses_dict['total_val_loss_list']
    ...
    for epoch in range(epochs):
        ...
        print('TRAINING')
        train_loss, mean_running_train_loss = train_epoch(device, train_loader, model, loss_fn, optimizer,
                                                          epochs_till_now, final_epoch, log_interval)
        print('VALIDATION')
        val_loss, mean_running_val_loss, roc_auc = val_epoch(device, val_loader, model, loss_fn, epochs_till_now,
                                                             final_epoch, log_interval)
        ...
        if ((epoch + 1) % save_interval == 0) or test_only:
            ...
            torch.save({
                ...
                'losses_dict': {'epoch_train_loss': epoch_train_loss, 'epoch_val_loss': epoch_val_loss,
        ...
        if scheduler:
            scheduler.step(mean_running_val_loss)  # Use validation loss or just step depending on scheduler type
